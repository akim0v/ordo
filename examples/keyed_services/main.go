...
func selectingByKey() {
	...
	c, err := ordo.New(
		...
		ordo.WithKeyedFactory("memory", NewMemoryCache),

		// An already-built value, under a key.
		ordo.WithKeyedValue[Cache]("null", &NullCache{}),
	)
	if err != nil {
...
func keysDoNotInject() {
	...
	_, err := ordo.New(
		ordo.WithKeyedService[Cache]("redis", NewRedisCache),
		ordo.WithFactory(NewSessionStore),
	)
...
func bothAtOnce() {
	...
	c, err := ordo.New(
		ordo.WithService[Cache](NewRedisCache),
